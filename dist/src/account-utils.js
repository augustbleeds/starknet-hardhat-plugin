...
exports.sendEstimateFeeTx = exports.sendDeclareV2Tx = exports.calculateDeclareV2TxHash = exports.sendDeployAccountTx = exports.calculateDeployAccountHash = exports.generateKeys = exports.handleInternalContractArtifacts = exports.signMultiCall = exports.generateRandomStarkPrivateKey = void 0;
...
function generateRandomStarkPrivateKey(length = 63) {
    ...
    for (let i = 0; i < length; ++i) {
        result += characters.charAt(crypto_1.default.randomInt(characters.length));
    }
    return starknet_1.number.toBN(result, "hex");
}
...
function signMultiCall(publicKey, keyPair, messageHash) {
    ...
    return starknet_1.ec.sign(keyPair, BigInt(messageHash).toString(16)).map(BigInt);
}
...
function ensureArtifact(fileName, artifactsTargetPath, artifactSourcePath) {
    ...
    if (!fs_1.default.existsSync(finalTargetPath)) {
        fs_1.default.mkdirSync(artifactsTargetPath, { recursive: true });
        const finalSourcePath = path_1.default.join(artifactSourcePath, fileName);
        fs_1.default.copyFileSync(finalSourcePath, finalTargetPath);
    }
}
/**
...
function generateKeys(providedPrivateKey) {
    const starkPrivateKey = providedPrivateKey
        ? starknet_1.number.toBN(providedPrivateKey.replace(/^0x/, ""), 16)
        : generateRandomStarkPrivateKey();
    const keyPair = starknet_1.ec.getKeyPair(starkPrivateKey);
    const publicKey = starknet_1.ec.getStarkKey(keyPair);
...
function calculateDeclareV2TxHash(accountAddress, callData, maxFee, chainId, additionalData) {
    const calldataHash = starknet_1.hash.computeHashOnElements(callData);
    return starknet_1.hash.computeHashOnElements([
        constants_1.TransactionHashPrefix.DECLARE,
        (0, utils_1.numericToHexString)(constants_1.DECLARE_VERSION),
        accountAddress,
        0,
        calldataHash,
        maxFee,
...
exports.calculateDeclareV2TxHash = calculateDeclareV2TxHash;
function sendDeclareV2Tx(signatures, classHash, maxFee, senderAddress, version, nonce, contractClass) {
    return __awaiter(this, void 0, void 0, function* () {
        const hre = yield Promise.resolve().then(() => __importStar(require("hardhat")));
        const resp = yield axios_1.default
            .post(`${hre.starknet.networkConfig.url}/gateway/add_transaction`, {
            ...
            compiled_class_hash: starknet_1.number.toHex(starknet_1.number.toBN(classHash)),
